...

{
	...
	{
		if(!$this->_db->delete('users', $where))
			return false;
		return true;
	}

	public function insertGroup($fields = array())
	{
		if (!$this->_db->insert('future_administration_groups', $fields))
			throw new Exception("There was a problem inserting.");
	}

	public function updateGroup($fields = array(), $id = null)
	{
		if (!$this->_db->update('future_administration_groups', $id, $fields))
			throw new Exception('There was a problem updating');
	}
