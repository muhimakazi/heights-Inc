...
class PaymentController
{
	...
	public static function sendPaymentReminder($transaction_status = 'APPROVED')
	{
		...
		if ($validate->passed()) {
			...
			$payment_entry_id = Hash::decryptToken($str->data_in($_PAY['Id']));
			$participant_id = Hash::decryptAuthToken($str->data_in($_PAY['participant']));
			$counter = $str->data_in($_PAY['counter']);

			// echo $participant_id;

			/** Get Participant Details */
			if (!($_participant_data_ = FutureEventController::getParticipantByID($participant_id))):
				return (object)[
					'ERRORS' => true,
					'ERRORS_SCRIPT' => "Invalid data",
					'ERRORS_STRING' => "Invalid data"
			...
			// echo json_encode($_participant_data_);

			// echo $_participant_data_->email;
			// echo $_participant_data_->firstname;
			...
			if ($diagnoArray[0] == 'NO_ERRORS') {
				...
				$_fields = array(
					'delayed_payment_reminder_status' => $counter + 1,
				);
