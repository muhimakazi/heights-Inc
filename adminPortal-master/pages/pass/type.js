$(document).ready(function () {
	...
	/** Add  */
	$('#addPassTypeForm').submit(function () {
		...
		$("#addPassTypeButton").button('loading');
		...
		$.ajax({
			...
			success: function (dataResponse) {
				...
				if (response.success == true) {
					$("#addPassTypeButton").button('reset');
					$("#addPassTypeForm")[0].reset();
					showPassTypesContent();
					...
					setTimeout(function () {
						$('#addPassTypeModal .close').delay(1200).click();
					}, 5000);
				} else {
					$("#addPassTypeButton").button('reset');
	...
	$(document).on('click', '.edit_type', function () {
		...
		var ePaymentStatus = $('#ePaymentStatus' + passId).text();
		var eVisibility = $('#eVisibility' + passId).text();
		var eForm = $('#eForm' + passId).text();
		
		$('#editModal').modal('show');
		$('#ename').val(name);
		$('#epayment_state').val(ePaymentStatus);
		$('#evisibility_state').val(eVisibility);
		$('#eform_order').val(eForm);
		$('#passId').val(passId);
	});
...
function showPassTypesContent() {
	$.ajax({
		type: 'POST',
		url: linkto,
		data: {request: "fetchPassTypes" },
		success: function (data) {
			$('#list-pass-types').html(data);
		}
	});
}