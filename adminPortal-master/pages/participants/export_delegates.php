...
if(!$user->isLoggedIn()) {
  ...
  // APPROVED
  $delegate_data = DB::getInstance()->query("SELECT * FROM `future_participants` WHERE `event_id` = $eventId AND `status` = 'APPROVED' ORDER BY firstname ASC");

  // ISSUE BADGE
  // $delegate_data = DB::getInstance()->query("SELECT * FROM `future_participants` WHERE `event_id` = $eventId AND `status` = 'APPROVED' AND `issue_badge_status` = 1 ORDER BY id ASC");
  ...
  if ($delegate_data->count()) {
    ...
    foreach ($delegate_data->results() as $tdelegate) {
      ...
      if(json_decode($fullDetails)!=null){
        ...
        $accommodation = property_exists($fullDetails, "book_room")?$fullDetails->book_room:"";
      ...
      // if(property_exists($fullDetails, "book_room") AND $accommodation == 'YES'){
        $i++;
        $delegates[] = array(
          $i,
          $tdelegate->title,
...
// }
  ...
  $filename = "DELEGATES_".date('Y-m-d H:i:s').".csv";
